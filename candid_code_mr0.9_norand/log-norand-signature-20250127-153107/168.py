...
REWARD_EVICT = -0.5  # Penalty for eviction
...
temporal_difference_error = collections.defaultdict(float)
candidate_policies = []
...
def update_after_hit(cache_snapshot, obj):
    ...
    temporal_difference_error[obj.key] = td_error
...
def update_after_insert(cache_snapshot, obj):
    ...
    # Your code below
    current_state = tuple(cache_snapshot.cache.keys())
    next_state = current_state + (obj.key,)
    reward = REWARD_MISS
    best_next_action_value = max(Q_table[obj.key].values(), default=0)
    td_target = reward + GAMMA * best_next_action_value
...
def update_after_evict(cache_snapshot, obj, evicted_obj):
    ...
    temporal_difference_error[evicted_obj.key] = td_error
