...
REWARD_EVICT = -0.5  # Reward for evicting an object
...
def update_after_hit(cache_snapshot, obj):
    ...
    reward = REWARD_HIT
    next_state = state
    best_next_action = max(Q_table[obj.key], key=Q_table[obj.key].get, default='hit')
    td_target = reward + GAMMA * Q_table[obj.key][best_next_action]
    td_error = td_target - Q_table[obj.key][action]
    Q_table[obj.key][action] += ALPHA * td_error
...
def update_after_insert(cache_snapshot, obj):
    ...
    best_next_action = max(Q_table[obj.key], key=Q_table[obj.key].get, default='insert')
    td_target = reward + GAMMA * Q_table[obj.key][best_next_action]
    td_error = td_target - Q_table[obj.key][action]
    Q_table[obj.key][action] += ALPHA * td_error
...
def update_after_evict(cache_snapshot, obj, evicted_obj):
    ...
    reward = REWARD_EVICT
    next_state = state
    best_next_action = max(Q_table[evicted_obj.key], key=Q_table[evicted_obj.key].get, default='evict')
    td_target = reward + GAMMA * Q_table[evicted_obj.key][best_next_action]
    td_error = td_target - Q_table[evicted_obj.key][action]
    Q_table[evicted_obj.key][action] += ALPHA * td_error
