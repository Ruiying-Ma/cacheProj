...
def update_after_hit(cache_snapshot, obj):
    ...
    reward = REWARD_HIT
    next_state = state
    current_q = Q_table[obj.key][action]
    max_next_q = max(Q_table[obj.key].values(), default=0)
    Q_table[obj.key][action] = current_q + ALPHA * (reward + GAMMA * max_next_q - current_q)
...
def update_after_evict(cache_snapshot, obj, evicted_obj):
    ...
    action = 'evict'
    reward = REWARD_EVICT
    next_state = tuple(key for key in state if key != evicted_obj.key)
    current_q = Q_table[evicted_obj.key][action]
    max_next_q = max(Q_table[evicted_obj.key].values(), default=0)
    Q_table[evicted_obj.key][action] = current_q + ALPHA * (reward + GAMMA * max_next_q - current_q)
    markov_chain[state][next_state] += 1