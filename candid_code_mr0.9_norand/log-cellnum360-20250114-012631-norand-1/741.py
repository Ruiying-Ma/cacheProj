...
def evict(cache_snapshot, obj):
    ...
    candidate_clusters = [key for key, score in pci.items() if score == min_pci_score]
    ...
    if candidate_clusters.count(candid_obj_key) > 1:
        min_qcm_score = min([qcm[key] for key in candidate_clusters if tar[key] == max_tar_score])
        for key in candidate_clusters:
            if tar[key] == max_tar_score and qcm[key] == min_qcm_score:
                candid_obj_key = key
                break
