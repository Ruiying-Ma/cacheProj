    ...
    simulator = SimulatorCache(
        SimulatorConfig(
            ...
            config=CacheConfig(
                ...
                trace_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "cache", "trace", "zipf", "alpha1_m100_n1000", "0.oracleGeneral.bin"),
                key_col_id=1,
                size_col_id=2,
                has_header=False,
                delimiter=","
...
def tune():
    ...
    with open(policy_path, 'r') as file:
        ...
        e = time.time()
        if mr_info != None:
            logging.info(f"Tune: time = {e-s}, tuned_mr = {mr_info[0]}, default_params = {mr_info[1]}, tuned_params = {mr_info[2]}")
        else:
